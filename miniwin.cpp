...
void dibujar_marco(int WIDTH, int HEIGHT, int margen, int espacio_superior) {
    ...
    color(BLANCO);
    // LÌneas del marco
    rectangulo_lleno(x_ini, y_ini, x_fin, y_ini + 2); // LÌnea superior
    rectangulo_lleno(x_ini, y_fin - 2, x_fin, y_fin); // LÌnea inferior
    rectangulo_lleno(x_ini, y_ini, x_ini + 2, y_fin); // LÌnea izquierda
    rectangulo_lleno(x_fin - 2, y_ini, x_fin, y_fin); // LÌnea derecha
}


//MÈtodo para dibujar nave
...
void dibujar_enemigo_alien(float x, float y, float longitud_cuerpo, float altura_cuerpo, float longitud_antena, float altura_antena) {
    // Coordenadas del cuerpo del alienÌgena
